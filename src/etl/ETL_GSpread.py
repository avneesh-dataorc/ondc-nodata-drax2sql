...
class ETLGSpread:
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        logger = logging.getLogger(f"{__name__}.ETLGSpread")
    ...
    def transform_rls_buyer_np_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Transform RLS Buyer NP data for database insertion.
        ...
        """
        try:
            self.logger.info("Starting RLS Buyer NP data transformation")
            ...
            self.logger.error(f"Error in RLS Buyer NP data transformation: {e}")
            raise
    
    def load_rls_buyer_np_data(self, spreadsheet_id: str, spreadsheet_range: str, target_table: str = None) -> bool:
        """
        Load RLS Buyer NP data from Google Spreadsheet to database.
        
        Args:
            spreadsheet_id: The ID of the Google Spreadsheet containing RLS Buyer NP data
            spreadsheet_range: The range to fetch from the spreadsheet
            target_table: Target table name (optional, uses TBL_RLS_BUYER_NP env var if not provided)
            
        Returns:
            bool: True if successful, False otherwise
        """
        try:
            target_table = target_table or os.getenv('TBL_RLS_BUYER_NP', 'rls_buyer_np')
            
            self.logger.info(f"Starting RLS Buyer NP data ETL process")
            self.logger.info(f"Spreadsheet ID: {spreadsheet_id}")
            self.logger.info(f"Spreadsheet Range: {spreadsheet_range}")
            self.logger.info(f"Target table: {target_table}")
            ...
            # Transform data
            df_transformed = self.transform_rls_buyer_np_data(df)
            ...
            if success:
                self.logger.info(f"Successfully loaded {len(df_transformed)} RLS Buyer NP records to {target_table}")
                return True
            else:
                self.logger.error("Failed to insert RLS Buyer NP data into database")
                return False
    ...
    def transform_seller_np_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Transform Seller NP data for database insertion.
        ...
        """
        try:
            self.logger.info("Starting Seller NP data transformation")
            
            # Map spreadsheet columns to database columns
            column_mapping = {
                'seller_np_name': 'seller_np_name',
                'sellernp': 'seller_np',
                'updateddate': 'updated_date',
                'snpmask': 'snp_mask',
                'tsppowered': 'tsp_powered',
            ...
            self.logger.info(f"Seller NP data transformation completed. Processed {len(df_final)} rows")
            return df_final
            
        except Exception as e:
            self.logger.error(f"Error in Seller NP data transformation: {e}")
            raise
    
    def load_seller_np_data(self, spreadsheet_id: str, spreadsheet_range: str, target_table: str = None) -> bool:
        """
        Load Seller NP data from Google Spreadsheet to database.
        
        Args:
            spreadsheet_id: The ID of the Google Spreadsheet containing Seller NP data
            spreadsheet_range: The range to fetch from the spreadsheet
            target_table: Target table name (optional, uses TBL_SELLER_NP env var if not provided)
            
        Returns:
            bool: True if successful, False otherwise
        """
        try:
            target_table = target_table or os.getenv('TBL_SELLER_NP', 'seller_np')
            
            self.logger.info(f"Starting Seller NP data ETL process")
            self.logger.info(f"Spreadsheet ID: {spreadsheet_id}")
            self.logger.info(f"Spreadsheet Range: {spreadsheet_range}")
            self.logger.info(f"Target table: {target_table}")
            ...
            # Fetch data from spreadsheet
            df = self.gsheets_client.fetch_sheet_data(spreadsheet_id, range_name=spreadsheet_range)
            # Rename first column from "seller_np" to "seller_np_name"
            df.rename(columns={df.columns[0]: 'seller_np_name'}, inplace=True)
            if df is None or df.empty:
                self.logger.error("Failed to fetch data or no data available")
                return False
            
            # Transform data
            df_transformed = self.transform_seller_np_data(df)
            ...
            if success:
                self.logger.info(f"Successfully loaded {len(df_transformed)} Seller NP records to {target_table}")
                return True
            else:
                self.logger.error("Failed to insert Seller NP data into database")
                return False
                
        except Exception as e:
            self.logger.error(f"Error in Seller NP data ETL process: {e}")
            return False

  
    def transform_buyer_np_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Transform Buyer NP data for database insertion.
        ...
        """
        try:
            self.logger.info("Starting Buyer NP data transformation")
            
            # Map spreadsheet columns to database columns
            # Note: gspread_util.py transforms column names (lowercase, no spaces, underscores for special chars)
            column_mapping = {
                'buyer_np_name': 'buyer_np_name',
                'buyernp': 'buyer_np',
                'updateddate': 'updated_date',
                'bnpmask': 'bnp_mask',
                'domain': 'domain',
            ...
            self.logger.info(f"Buyer NP data transformation completed. Processed {len(df_final)} rows")
            return df_final
            
        except Exception as e:
            self.logger.error(f"Error in Buyer NP data transformation: {e}")
            raise
    
    def load_buyer_np_data(self, spreadsheet_id: str, spreadsheet_range: str, target_table: str = None) -> bool:
        """
        Load Buyer NP data from Google Spreadsheet to database.
        
        Args:
            spreadsheet_id: The ID of the Google Spreadsheet containing Buyer NP data
            spreadsheet_range: The range to fetch from the spreadsheet
            target_table: Target table name (optional, uses TBL_BUYER_NP env var if not provided)
            
        Returns:
            bool: True if successful, False otherwise
        """
        try:
            target_table = target_table or os.getenv('TBL_BUYER_NP', 'buyer_np')
            
            self.logger.info(f"Starting Buyer NP data ETL process")
            self.logger.info(f"Spreadsheet ID: {spreadsheet_id}")
            self.logger.info(f"Spreadsheet Range: {spreadsheet_range}")
            self.logger.info(f"Target table: {target_table}")
            ...
            # Fetch data from spreadsheet
            df = self.gsheets_client.fetch_sheet_data(spreadsheet_id, range_name=spreadsheet_range)
            # Rename first column from "buyer_np" to "buyer_np_name"
            df.rename(columns={df.columns[0]: 'buyer_np_name'}, inplace=True)
            if df is None or df.empty:
                self.logger.error("Failed to fetch data or no data available")
                return False
            
            # Transform data
            df_transformed = self.transform_buyer_np_data(df)
            ...
            if success:
                self.logger.info(f"Successfully loaded {len(df_transformed)} Buyer NP records to {target_table}")
                return True
            else:
                self.logger.error("Failed to insert Buyer NP data into database")
                return False
                
        except Exception as e:
            self.logger.error(f"Error in Buyer NP data ETL process: {e}")
            return False


def run_seller_np_etl():
    """
    Main function to run the Seller NP ETL process.
    This function uses environment variables for configuration.
    """
    try:
        # Get configuration from environment variables
        config_dir = os.getenv('CONFIG_DIR')
        spreadsheet_id = os.getenv('SPREAD_SHEET_SELLER_NP')
        spreadsheet_range = os.getenv('SPREAD_SHEET_SELLER_NP_RANGE')
        target_table = os.getenv('TBL_SELLER_NP', 'seller_np')
        
        if not config_dir:
            raise ValueError("CONFIG_DIR environment variable is required")
        if not spreadsheet_id:
            raise ValueError("SPREAD_SHEET_SELLER_NP environment variable is required")
        if not spreadsheet_range:
            raise ValueError("SPREAD_SHEET_SELLER_NP_RANGE environment variable is required")
        
        # Initialize ETL processor
        etl = ETLGSpread(config_dir)
        
        # Run the ETL process
        success = etl.load_seller_np_data(spreadsheet_id, spreadsheet_range, target_table)
        
        if success:
            print("Seller NP ETL process completed successfully!")
            return True
        else:
            print("Seller NP ETL process failed!")
            return False
            
    except Exception as e:
        print(f"Error in Seller NP ETL process: {e}")
        return False
...
def run_rls_buyer_np_etl():
    """
    Main function to run the RLS Buyer NP ETL process.
    This function uses environment variables for configuration.
    """
    try:
        # Get configuration from environment variables
        config_dir = os.getenv('CONFIG_DIR')
        spreadsheet_id = os.getenv('SPREAD_SHEET_RLS_BUYER_NP')
        spreadsheet_range = os.getenv('SPREAD_SHEET_RLS_BUYER_NP_RANGE')
        target_table = os.getenv('TBL_RLS_BUYER_NP', 'rls_buyer_np')
        
        if not config_dir:
            raise ValueError("CONFIG_DIR environment variable is required")
        if not spreadsheet_id:
            raise ValueError("SPREAD_SHEET_RLS_BUYER_NP environment variable is required")
        if not spreadsheet_range:
            raise ValueError("SPREAD_SHEET_RLS_BUYER_NP_RANGE environment variable is required")
        
        # Initialize ETL processor
        etl = ETLGSpread(config_dir)
        
        # Run the ETL process
        success = etl.load_rls_buyer_np_data(spreadsheet_id, spreadsheet_range, target_table)
        
        if success:
            print("RLS Buyer NP ETL process completed successfully!")
            return True
        else:
            print("RLS Buyer NP ETL process failed!")
            return False
            
    except Exception as e:
        print(f"Error in RLS Buyer NP ETL process: {e}")
        return False
...
def run_buyer_np_etl():
    """
    Main function to run the Buyer NP ETL process.
    This function uses environment variables for configuration.
    """
    try:
        # Get configuration from environment variables
        config_dir = os.getenv('CONFIG_DIR')
        spreadsheet_id = os.getenv('SPREAD_SHEET_BUYER_NP')
        spreadsheet_range = os.getenv('SPREAD_SHEET_BUYER_NP_RANGE')
        target_table = os.getenv('TBL_BUYER_NP', 'buyer_np')
        
        if not config_dir:
            raise ValueError("CONFIG_DIR environment variable is required")
        if not spreadsheet_id:
            raise ValueError("SPREAD_SHEET_BUYER_NP environment variable is required")
        if not spreadsheet_range:
            raise ValueError("SPREAD_SHEET_BUYER_NP_RANGE environment variable is required")
        
        # Initialize ETL processor
        etl = ETLGSpread(config_dir)
        
        # Run the ETL process
        success = etl.load_buyer_np_data(spreadsheet_id, spreadsheet_range, target_table)
        
        if success:
            print("Buyer NP ETL process completed successfully!")
            return True
        else:
            print("Buyer NP ETL process failed!")
            return False
            
    except Exception as e:
        print(f"Error in Buyer NP ETL process: {e}")
        return False
